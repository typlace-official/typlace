(function(){
  ...
  function getCur(){
    return localStorage.getItem(KEY_CUR) || "";
  }

  function setCur(v){
    localStorage.setItem(KEY_CUR, v);
  }
  ...
  async function initHeader(){
    ...
    function getActiveSearchInput(){
      return window.innerWidth <= 640 ? sInputMobile : sInputDesktop;
    }
    ...
    if (burgerLangToggle && burgerCurToggle) {
      function updateBurgerValues() {
        ...
        const curItem = (CURRENCIES_BY_LANG[lang] || []).find(x => x.id === cur);
  ...
  window.tpHeaderReady = mountHeader()
    .catch((err) => {
      console.error("Header mount error:", err);
    })
    .finally(() => {
      window.dispatchEvent(new CustomEvent("tp:header-ready"));
    });
